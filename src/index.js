	...
	if (config.messages.autoRemind) {
		setInterval(() => {
			if (moment.tz(config.timezone).format("HH:mm") == config.messages.autoRemindTime) {
				bot.sendMessage(
					config.messages.chatId,
					"It's time to sign a Covid-19 digital health statement!\n\nPress /sign to do that."
				)
			}
		}, 60000)
	}
	...
	if (pages.length > 0) {
		...
		try {
			...
			if (bot != null)
				bot.sendMessage(
					config.messages.chatId,
					"Looks like your username or password to smartschool is wrong. Please check the configuration file."
				)
		...
		try {
			...
			if (bot != null)
				bot.sendMessage(
					config.messages.chatId,
					"Looks like you can 't sign a Covid-19 digital health statement right now."
				)
